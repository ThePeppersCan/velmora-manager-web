...
(() => {
  ...
  function careerStat(value,base=60){const candidate=Number(value??base),n=Number.isFinite(candidate)?candidate:60;return clamp(.70+((clamp(n,30,99)-40)/59)*.285,.65,.985);}
  function careerAttributesFromStats(player){const st=player?.stats||{},form=({Excellent:.018,Good:.009,Average:0,'New Signing':.004,Poor:-.012,Terrible:-.022}[player?.form]||0),morale=({'Very Happy':.012,Happy:.006,Content:0,Unhappy:-.010,'Very Unhappy':-.018,Excellent:.012,Good:.006,Okay:0,Poor:-.010,Bad:-.018}[player?.morale]||0),ov=careerStat(player?.ovr,65),fitness=clamp(Number(player?.fitness??82),45,100);const tune=(value,extra=0)=>clamp(value+form+morale+extra,.60,.99);const readiness=clamp(((Number.isFinite(Number(player?.sharpness))?Number(player.sharpness):70)-70)*.0006,-.035,.018),ready=(value)=>tune(value,readiness);return{speed:tune(careerStat(st.PAC,player?.ovr)),accel:tune(careerStat(st.PAC,player?.ovr),.006),turn:tune((careerStat(st.PAS,player?.ovr)+ov)/2),passing:ready(careerStat(st.PAS,player?.ovr)),catching:ready(careerStat(st.HAN,player?.ovr)),shooting:ready(careerStat(st.SHO,player?.ovr)),interception:ready(careerStat(st.DEF,player?.ovr)),awareness:ready((careerStat(st.DEF,player?.ovr)+ov)/2),positioning:ready((careerStat(st.DEF,player?.ovr)+ov)/2),reaction:ready((careerStat(st.PAC,player?.ovr)+ov)/2),anticipation:ready((careerStat(st.DEF,player?.ovr)+careerStat(st.PAS,player?.ovr))/2),decision:ready((careerStat(st.PAS,player?.ovr)+ov)/2),composure:ready(ov),aggression:tune(.82),stamina:tune(careerStat(st.STA,player?.ovr),(fitness-82)/900),recovery:tune(careerStat(st.STA,player?.ovr),(fitness-82)/950)};}
  ...
  function careerTacticalProfile(raw={}){
    const defensive=String(raw.defensive||'Balanced'),attacking=String(raw.attacking||'Balanced'),mentality=String(raw.mentality||'Balanced');
    ...
    p.name=`${defensive} · ${attacking} · ${mentality}`.toUpperCase();p.short=`${defensive.toLowerCase()} / ${attacking.toLowerCase()}`;
    p.comment=`are playing ${attacking.toLowerCase()} Quidditch with a ${defensive.toLowerCase()} defensive shape and ${mentality.toLowerCase()} mentality.`;
    return p;
  }
...
function ensureBigMomentStyles(){
  ...
  const MATCHDAY_TACTICS = {defensive:['Balanced','Press','Drop Back'],attacking:['Balanced','Fast Break','Possession','Direct'],mentality:['Defensive','Balanced','Attacking']};
  ...
  function matchdayEnergyStep(dt){
    ...
    for(const e of state.entities){const r=m.players[e.player.id];if(!r)continue;
      const t=m.tactics[e.team],work=(t.defensive==='Press'?1.3:t.defensive==='Drop Back'?.91:1)*(t.mentality==='Attacking'?1.08:1)*(t.attacking==='Fast Break'?1.08:1);
  ...
  const audio = {
    crowd:null,whistle:null,goal:null,goalCheer:null,prematch:null,kickoffRelease:null,matchMusic:null,matchMusicAlt:null,matchMusicThird:null,matchMusicFourth:null,matchMusicTracks:[],currentMatchMusic:null,currentMatchMusicIndex:-1,matchMusicFading:false,matchMusicShuffleBag:[],lastMatchMusicIndex:-1,intercepts:[],shots:[],throwSfx:null,catchSfx:null,passMissSfx:[],missImpactSfx:[],hoopHitSfx:[],reboundSfx:[],stealSfx:[],foulSfx:null,tackleSfx:null,collisionSfx:null,broomFlybySfx:[],fastBroomFlybySfx:null,lastBroomFlybyAt:0,lastChallengeAt:0,crowdAccentSfx:[],crowdLong:null,crowdAccentTimer:18,crowdAccentActive:null,crowdAccentStopTimer:0,lastCrowdAccent:-1,goalCheerGuardUntil:0,activeTransientSfx:new Set(),windCtx:null,windSource:null,mix:{ambience:.018,crowd:.18,commentary:.58,sfx:.55,music:.20,matchMusic:.05},
      ...
      this.broomFlybySfx=[1,2].map(n=>new Audio(`assets/quidditch-sfx/broom-flyby-${n}.mp3`));
      this.fastBroomFlybySfx=new Audio('assets/quidditch-sfx/broom-flyby-fast.mp3');
      ...
      this.crowdAccentSfx=[];
      this.crowdLong=null;
      [this.throwSfx,this.catchSfx,this.foulSfx,this.tackleSfx,this.collisionSfx,this.fastBroomFlybySfx,...this.intercepts,...this.shots,...this.passMissSfx,...this.missImpactSfx,...this.hoopHitSfx,...this.reboundSfx,...this.stealSfx,...this.broomFlybySfx].forEach(a=>{if(a)a.preload='auto'});
    },
    ...
    broomFlyby(fast=false,intensity=1){
      this.ensure();const now=(typeof performance!=='undefined'&&performance.now)?performance.now():Date.now(),cooldown=fast?850:1250;if(now-this.lastBroomFlybyAt<cooldown)return;this.lastBroomFlybyAt=now;
      let a=this.fastBroomFlybySfx;if(!fast&&this.broomFlybySfx.length)a=this.broomFlybySfx[Math.floor((state.audioRand?.()||Math.random())*this.broomFlybySfx.length)];
      this.play(a,clamp((fast?.34:.25)*clamp(Number(intensity)||1,.65,1.25),.14,.43),{rate:.96+(state.audioRand?.()||Math.random())*.08,fadeIn:18,fadeOut:140});
    },
