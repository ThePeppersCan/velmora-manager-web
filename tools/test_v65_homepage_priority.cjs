...
assert.match(index,new RegExp(`homepage-polish\.css\\?v=${RELEASE_CACHE_KEY}`),'homepage stylesheet uses the unified release cache key');
...
for(const label of ['SUPPORTERS','DRESSING ROOM','BOARD','PRESS'])assert.ok(app.includes(`label:'${label}'`),`Club Pulse metric: ${label}`);
