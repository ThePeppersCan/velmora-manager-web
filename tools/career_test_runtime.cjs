...
const scripts=['release-meta.js','clubs.js','names.js','assets/manager/manager-assets.js','world-expansion.js','data/player-sprites.js','data/club-stadiums.js','training.js','player-performance.js','training-ui.js','vendor/pako.min.js','save-codec.js','career-expansion.js','player-traits.js','player-profiles.js','negotiation-cinematic.js','chairmen.js','club-pulse.js','multiplayer-core.js','app.js'].filter(f=>fs.existsSync(path.join(root,f)));
for(const file of scripts){let source=fs.readFileSync(path.join(root,file),'utf8');if(file==='app.js'){
 ...
 source=source.replace('  window.VELMORA_MANAGER_DEBUG={',`window.auditQA={${[...new Set(names)].join(',')},state:()=>({worldSeed,fixtures,careerTime,careerRuntime,livingSquad,roadToGlory,currentClub,careerPreferences,employmentStatus,careerMode,playerCareerState,clubs,managerMarket,ownershipState,audienceWorldState,calendarEvents,squadCache,lineupCache,freeAgentCache,youthAcademies,transferActivity}),set:(key,value)=>{switch(key){case 'currentClub':currentClub=value;break;case 'fixtures':fixtures=value;v202MarkCompetitionDataDirty();break;case 'employmentStatus':employmentStatus=value;break;case 'careerMode':careerMode=value;break;case 'playerCareerState':playerCareerState=value;break;}}};\n  window.VELMORA_MANAGER_DEBUG={`);
