...
const ctx=vm.createContext({console,Math,Number,Map,Set,trainingRules:require('../training.js')});
...
if(require.main===module)(async()=>{
 ...
 // Raw simulation quality: fixed lineup, world effects stubbed, actual score-generating function.
 const squads={},home={id:'H'},away={id:'A'};let currentFixture;
 Object.assign(ctx,{worldSeed:'QUALITY-AUDIT',currentClub:home,clubs:[home,away],getSquad:c=>squads[c.id],activeStarters:c=>squads[c.id],aiTacticalCohesionModifier:()=>0,moraleIndex:()=>2,prepareAiLineupForFixture:()=>{},fixtureClubs:()=>({home,away}),resolveChampionsCrownDecider:()=>{},v202MarkCompetitionDataDirty:()=>{},applyBackgroundMatchEffects:()=>{},ensurePlayerCareerMeta:()=>{},championsCrownAfterFixture:()=>{}});
 for(const name of ['v24PlayerStat','v24TeamProfile','v24ExpectedGoals','v24ScorerWeight','matchMoraleModifier','matchFitnessModifier','backgroundClubStrength','backgroundGoals','v23TeamSharpness','weightedScorer','simulateBackgroundFixture'])if(source.includes('  function '+name+'('))vm.runInContext(fn(name),ctx);
 const sims=[];
 for(const settings of [{name:'OVR 90 v 55',strong:90,weak:55},{name:'Same OVR 75; all attributes 90 v 50',strong:75,weak:75,stats:true}]){
  ...
  for(let i=0;i<total;i++){
   ...
   currentFixture={fixtureId:'SIM-'+i,homeClubId:'H',awayClubId:'A',date:'2026-09-01',type:'LEAGUE'};ctx.simulateBackgroundFixture(currentFixture);const a=side==='H'?currentFixture.homeScore:currentFixture.awayScore,b=side==='H'?currentFixture.awayScore:currentFixture.homeScore;wins+=a>b;draws+=a===b;losses+=a<b;gf+=a;ga+=b;
