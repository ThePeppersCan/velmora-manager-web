...
(async()=>{
 ...
 const timeoutStore=create({indexedDB:new IDBFactory(),localStorage:{getItem:()=>null,setItem(){},removeItem(){}},codec,transactionTimeoutMs:25});await timeoutStore.ready;
 IDBDatabase.prototype.transaction=function(...args){if(args[1]!=='readwrite')return originalTransaction.apply(this,args);return{error:null,abort(){},objectStore(){return{put(){},delete(){}}}};};
 try{timeoutStore.setItem(PREFIX+1,raw(8));const started=Date.now();await assert.rejects(timeoutStore.flush(),/timed out/i);assert(Date.now()-started<1000,'A stalled browser transaction fails promptly instead of leaving the UI on Saving career');}
 finally{IDBDatabase.prototype.transaction=originalTransaction;}
 console.log(JSON.stringify({status:'PASS',checks:['migration removes local bytes only after durable commit','five 3 MB careers exceed localStorage quota without failure','all slots survive reopen','previous save recovery','invalid import preserves destination','backup export/import','deleted slots cannot resurrect from legacy bytes','aborted transaction preserves durable save and exports unsaved progress','successful retry clears failure','stalled writes time out with an actionable error']},null,2));
