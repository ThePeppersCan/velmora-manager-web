...
const root=path.resolve(__dirname,'..'),T=require('../training.js'),app=fs.readFileSync(path.join(root,'app.js'),'utf8');
...
assert(!app.includes('Training remains intentionally lightweight'));assert(app.includes('matchFitnessModifier(home)+v23TeamSharpness(home)'));assert(app.includes('const expected=v24ExpectedGoals(home,away)')); 
