...
check('all expansion state survives career save/load and slot switching',()=>{
  assert(q.saveCareerState());const original=JSON.stringify(x.state()),staff=x.department().staff.length;assert(q.loadCareerState());assert.equal(x.department().staff.length,staff);assert.equal(JSON.stringify(x.state()),original);
});
