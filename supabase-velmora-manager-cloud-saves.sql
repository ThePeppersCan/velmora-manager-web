...
values ('velmora-manager-careers', 'velmora-manager-careers', false, 8388608, array['application/octet-stream', 'text/plain'])
...
create policy "Velmora players read their career slots" on public.velmora_manager_career_slots
for select to authenticated using ((select auth.uid()) = user_id);

drop policy if exists "Velmora players create their career slots" on public.velmora_manager_career_slots;
create policy "Velmora players create their career slots" on public.velmora_manager_career_slots
for insert to authenticated with check ((select auth.uid()) = user_id);

drop policy if exists "Velmora players update their career slots" on public.velmora_manager_career_slots;
create policy "Velmora players update their career slots" on public.velmora_manager_career_slots
for update to authenticated using ((select auth.uid()) = user_id) with check ((select auth.uid()) = user_id);

drop policy if exists "Velmora players delete their career slots" on public.velmora_manager_career_slots;
create policy "Velmora players delete their career slots" on public.velmora_manager_career_slots
for delete to authenticated using ((select auth.uid()) = user_id);

drop policy if exists "Velmora players read their career objects" on storage.objects;
create policy "Velmora players read their career objects" on storage.objects
for select to authenticated using (bucket_id = 'velmora-manager-careers' and (storage.foldername(name))[1] = (select auth.uid())::text);

drop policy if exists "Velmora players create their career objects" on storage.objects;
create policy "Velmora players create their career objects" on storage.objects
for insert to authenticated with check (bucket_id = 'velmora-manager-careers' and (storage.foldername(name))[1] = (select auth.uid())::text);

drop policy if exists "Velmora players update their career objects" on storage.objects;
create policy "Velmora players update their career objects" on storage.objects
for update to authenticated
using (bucket_id = 'velmora-manager-careers' and (storage.foldername(name))[1] = (select auth.uid())::text)
with check (bucket_id = 'velmora-manager-careers' and (storage.foldername(name))[1] = (select auth.uid())::text);

drop policy if exists "Velmora players delete their career objects" on storage.objects;
create policy "Velmora players delete their career objects" on storage.objects
for delete to authenticated using (bucket_id = 'velmora-manager-careers' and (storage.foldername(name))[1] = (select auth.uid())::text);
