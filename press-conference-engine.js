(function(){
  ...
  function create(adapter={}){
    ...
    function buildQuestions(context){
      const stage=context.stage,rng=rngFrom(`${context.fixtureId}-${stage}-${context.date}-${library.version||1}`);
      const all=(library.categories||[]).filter(c=>categoryAvailable(c,stage,context));
      const desired=4;
      const usedCategories=new Set(),usedKeys=new Set(adapter.recentQuestionKeys?.()||[]),questions=[];
      let guard=0;
      while(questions.length<desired&&guard++<180&&all.length){
        const candidates=all.filter(c=>!usedCategories.has(c.id));
        const category=weightedCategory(candidates.length?candidates:all,rng);if(!category)break;
        const target=questionTarget(category,context,rng),templates=category.templates||[];
        if(!templates.length){usedCategories.add(category.id);continue;}
        let templateIndex=Math.floor(rng()*templates.length),attempts=0,key='';
        do{key=`${category.id}:${templateIndex}:${target?.id||'GENERAL'}`;if(!usedKeys.has(key))break;templateIndex=(templateIndex+1)%templates.length;}while(++attempts<templates.length);
        ...
        questions.push({id:`Q${questions.length+1}-${hash(key).toString(36)}`,key,categoryId:category.id,categoryLabel:category.label,targetId:target?.id||null,targetName:target?.name||null,reporter,text:frameQuestion(base,stage,reporter,context,rng),isFollowUp:false});
        usedCategories.add(category.id);usedKeys.add(key);
      }
      return questions;
    }
    ...
    // One grounded, topic-specific statement per question category. This is what keeps every
    // answer tied to the actual question asked, regardless of which tone the manager picks.
    const TOPIC_LINES={
    ...
    function responseText(choice,question,context,index){
      const subject=responseSubject(question,context);
      const tokens={...questionTokens(context,{name:subject,id:question.targetId}),subject};
      const rng=rngFrom(`${question.id}-${choice.id}-${context.fixtureId}-${index}`);
      const baseCategoryId=String(question.categoryId||'').replace(/^FOLLOW-UP · /,'');
      const topicFn=TOPIC_LINES[baseCategoryId]||TOPIC_LINES[question.categoryId]||(t=>`${question.categoryLabel||'this question'} is something I take seriously at ${t.club}`);
      const topic=cap(topicFn(tokens));
      const frame=TONE_FRAMES[choice.id]||TONE_FRAMES.accountable;
      const opener=pick(frame.openers,rng,frame.openers[0]);
      const closer=pick(frame.closers,rng,frame.closers[0]);
      return `${opener} ${topic}. ${closer}`;
    }
    function answersFor(question,context){
      const archetypes=library.responseArchetypes||DEFAULT_ARCHETYPES;
      return archetypes.slice(0,6).map((choice,index)=>({...choice,text:responseText(choice,question,context,index)}));
    }
    function createSession(context){
      ...
      return{id,key:sessionKey(context.stage,context.fixtureId),fixtureId:context.fixtureId,stage:context.stage,date:context.date,status:'INVITED',questionIndex:0,questions,answers:[],startedAt:null,completedAt:null,skipped:false,followUpUsed:false,impactTotals:{positive:0,negative:0},version:library.version||'1'};
    }
    ...
    function questionMarkup(context){
      ...
      return baseScene(context,`<section class="pc-question-card ${question.isFollowUp?'is-follow-up':''}"><div class="pc-reporter"><div class="pc-reporter-avatar">${avatar}</div><div class="pc-reporter-copy"><small>${esc(reporter.outlet||'VELMORA SPORT')}</small><strong>${esc(reporter.name||'PRESS ROOM REPORTER')} · ${esc(reporter.role||'MATCH CORRESPONDENT')}</strong></div><div class="pc-reporter-access"><span>MEDIA RELATIONSHIP</span><b>${relationshipLabel(reporter.relationship)}</b></div></div><div class="pc-topic">${esc(question.categoryLabel||'MATCHDAY')}</div><h2>${esc(question.text)}</h2></section><section class="pc-answer-panel"><div class="pc-answer-label"><span>CHOOSE YOUR RESPONSE</span><small>Every answer becomes part of your public record</small></div><div class="pc-answer-grid">${answers.map((answer,i)=>`<button type="button" class="pc-answer" data-pc-answer="${esc(answer.id)}"><b class="pc-answer-key">${i+1}</b><strong>${esc(answer.label)}</strong><small>${esc(answer.text)}</small></button>`).join('')}</div></section>`);
    }
    function responseMarkup(context,question,choice,text,impact){
      ...
      return baseScene(context,`<section class="pc-question-card ${question.isFollowUp?'is-follow-up':''}"><div class="pc-reporter"><div class="pc-reporter-avatar">${question.reporter?.avatar?`<img src="${esc(question.reporter.avatar)}" alt="">`:''}</div><div class="pc-reporter-copy"><small>${esc(question.reporter?.outlet||'VELMORA SPORT')}</small><strong>${esc(question.reporter?.name||'PRESS ROOM REPORTER')}</strong></div><div class="pc-reporter-access"><span>REACTION</span><b>${esc(impact?.reaction||'NOTED')}</b></div></div><div class="pc-topic">${esc(question.categoryLabel)}</div><h2>${esc(question.text)}</h2></section><section class="pc-response-panel"><span>${esc(choice.stance||choice.label)} · ${esc(context.managerName||'MANAGER')}</span><blockquote>“${esc(text)}”</blockquote><div class="pc-impact-row">${chips}</div><div class="pc-response-actions"><button type="button" class="pc-next" data-pc-next>${active.questionIndex>=active.questions.length-1?'FINISH CONFERENCE':'NEXT QUESTION'}</button></div></section><div class="pc-quote-ticker is-visible">${esc(impact?.ticker||`${question.reporter?.outlet||'The press room'} are already shaping the headline.`)}</div>`);
    }
    function completeMarkup(context){
      const answered=active.answers.length,positive=active.impactTotals?.positive||0,negative=active.impactTotals?.negative||0,mood=positive-negative>=3?'ROOM WON OVER':positive-negative<=-2?'TENSE EXIT':'PROFESSIONAL CLOSE';
      return baseScene(context,`<section class="pc-complete"><div class="pc-complete-card"><main class="pc-complete-main"><span>MEDIA DUTY COMPLETE</span><h1>${mood}</h1><p>${answered} question${answered===1?'':'s'} answered. Your comments are now part of the living career record and may return in future stories or follow-up questions.</p><div class="pc-summary-grid"><div><span>PRESS ROOM</span><strong>${mood}</strong></div><div><span>PUBLIC STANCE</span><strong>${esc(active.answers.at(-1)?.stance||'MEASURED')}</strong></div><div><span>QUESTIONS</span><strong>${answered}/${active.questions.length}</strong></div><div><span>CAREER MEMORY</span><strong>STATEMENT SAVED</strong></div></div></main><aside class="pc-complete-side"><div class="pc-duty-list"><div><b>✓</b><span><strong>DRESSING ROOM UPDATED</strong><small>Relevant players and personalities reacted to your words.</small></span></div><div><b>✓</b><span><strong>MEDIA RELATIONSHIPS UPDATED</strong><small>Reporters remember access, honesty and confrontation.</small></span></div><div><b>✓</b><span><strong>FUTURE STORIES ARMED</strong><small>Bold promises can be revisited if results turn.</small></span></div></div><div class="pc-intro-actions"><button type="button" class="is-primary" data-pc-finish>${esc(continueLabel)}</button></div></aside></div></section>`);
    }
    ...
    function maybeCreateFollowUp(question,choice,context,impact){
      if(active.followUpUsed||active.questionIndex>=active.questions.length-1)return;
      const provocative=['bold','guarded','demanding'].includes(choice.id),rng=rngFrom(`${active.id}-${question.id}-${choice.id}-FOLLOW`);
      if((provocative ? .62 : .24)<=rng())return;
      const next=active.questions[active.questionIndex+1],subject=responseSubject(question,context),lines=library.followUps?.[choice.id]||[
        `You have been very clear about ${subject}. What do you say to people who believe the evidence points the other way?`,
        `Just to press you on that answer: what happens if ${subject} cannot deliver the response you expect?`
      ];
      next.text=replaceTokens(pick(lines,rng,lines[0]),{subject,club:context.club.name,opponent:context.opponent.name,player:question.targetName||'the player',manager:context.managerName});
      next.categoryLabel=`FOLLOW-UP · ${question.categoryLabel}`;next.reporter={...question.reporter,relationship:impact?.relationship??question.reporter.relationship};next.isFollowUp=true;next.targetId=question.targetId;next.targetName=question.targetName;active.followUpUsed=true;
    }
    function answer(choiceId){
      if(active.status!=='ACTIVE'||active.awaitingNext)return;
      const question=active.questions[active.questionIndex],choice=(library.responseArchetypes||DEFAULT_ARCHETYPES).find(x=>x.id===choiceId);if(!question||!choice)return;
      ...
      active.answers.push({questionId:question.id,questionKey:question.key,categoryId:question.categoryId,reporterId:question.reporter?.id||null,choiceId:choice.id,stance:choice.stance||choice.label,text,targetId:question.targetId||null,impact,date:active.context.date});
