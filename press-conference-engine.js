(function(){
  ...
  function create(adapter={}){
    ...
    function questionEvidence(category,context,target){
      ...
      const history=target?.publicHistory?` · ${target.publicHistory}`:'';
      if(target&&category.id==='CONTRACTS')return`${target.name} · ${role} · ${Number(target.contractYears||0)} YEAR${Number(target.contractYears||0)===1?'':'S'} REMAINING${history}`;
      if(target&&['TRANSFER_RUMOUR','TRANSFERS'].includes(category.id))return`${target.name} · ${role} · ${String(target.transferStatus||'FUTURE UNDER DISCUSSION').replace(/_/g,' ')}${history}`;
      if(target&&category.id==='AVAILABILITY')return`${target.name} · ${role} · UNAVAILABLE FOR ${context.opponent.name}${history}`;
      const playerLine=target?`${target.name} · ${role}${target.form?` · ${target.form} form`:''}${appearances?` · ${appearances} apps${starts?` / ${starts} starts`:''}`:''}${goals?` · ${goals} goals`:''}${target.publicHistory?` · ${target.publicHistory}`:''}`:'';
      ...
      if(['RIVAL_MANAGER','RIVALRY_FIXTURE'].includes(category.id)&&context.rivalManager)return`${context.rivalManager.name} · ${context.opponent.name} · ${context.rivalManager.style||'MEASURED'}${context.rivalManager.publicHistory?` · ${context.rivalManager.publicHistory}`:''}`;
