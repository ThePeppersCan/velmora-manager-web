(function(root){
  'use strict';
  function create(a){
    const roles={scout:'Scout',coach:'First-team coach',medical:'Medical specialist',academy:'Academy coach',commercial:'Commercial director'};
    ...
    function state(){const r=a.runtime();return r.expansion||(r.expansion={version:1,clubs:{},customClubs:{},precontracts:[],sequence:0});}
    ...
    function department(club=a.club()){
      ...
      const s=state();
      if(!s.clubs[club.id]){
        const scouts=a.legacyScouts(club).map((p,i)=>({...p,key:'legacy-'+club.id+'-'+i,role:'scout',slot:i,wage:350+i*75,joined:a.date(),xp:0,course:null}));
        const quality=bounded(35+num(club.reputation,1)*7,35,80);
        const staff=[...scouts,...['coach','medical','academy','commercial'].map((role,i)=>({id:'staff-'+club.id+'-'+role,key:'staff-'+club.id+'-'+role,name:staffName(club.id+role),role,quality,judgement:3,network:3,specialism:role==='academy'?'YOUTH':'TECHNICAL',wage:300+i*100,joined:a.date(),xp:0,course:null}))];
    ...
    function market(club=a.club()){
      const d=department(club),month=a.date().slice(0,7);if(d.market.month!==month){
        d.market={month,candidates:Object.keys(roles).flatMap((role)=>Array.from({length:3},(_,i)=>{
          ...
          return {id:'candidate-'+key,key:'candidate-'+key,name:staffName(key),role,quality,wage,fee:wage*8,judgement:bounded(Math.round(quality/19),1,5),network:bounded(Math.round((quality+8)/20),1,5),specialism:specialisms[Math.floor(r()*specialisms.length)],xp:0,course:null};
        }))};
      }return d.market.candidates;
    }
    function hire(candidateId){const c=a.club();if(!c)return fail('Take a club job first.');const d=department(c),p=market(c).find(x=>x.id===candidateId);if(!p)return fail('This candidate is no longer available.');const limit=p.role==='scout'?5:2;if(d.staff.filter(x=>x.role===p.role).length>=limit)return fail('This department has no vacant positions.');if(!transact(c,-p.fee,'Staff signing: '+p.name))return fail('Insufficient transfer budget for the signing fee.');d.staff.push({...p,joined:a.date(),...(p.role==='scout'?{slot:d.nextScoutSlot++}: {})});d.market.candidates=d.market.candidates.filter(x=>x.id!==p.id);notice(c,p.name+' joins the staff',roles[p.role]+' · '+a.format(p.wage)+' per week.');return success('Staff member hired.');}
    ...
    function normalTerms(input){return {fee:a.money(input.fee),wage:a.money(input.wage),bonus:a.money(input.bonus),years:Number(input.years),role:input.role||'Rotation',sellOn:Number(input.sellOn||0),releaseClause:a.money(input.releaseClause),swapId:input.swapId||null,optionFee:a.money(input.optionFee),wageShare:Number(input.wageShare??100)};}
    function termsValid(t){return [t.fee,t.wage,t.bonus,t.releaseClause,t.optionFee].every(v=>Number.isFinite(v)&&v>=0)&&Number.isInteger(t.years)&&t.years>=1&&t.years<=5&&['Crucial','Important','Rotation','Prospect','Reserve','Sporadic'].includes(t.role)&&Number.isFinite(t.sellOn)&&t.sellOn>=0&&t.sellOn<=30&&Number.isFinite(t.wageShare)&&t.wageShare>=0&&t.wageShare<=100;}
    function proposal(playerId,kind,input){
      ...
      const cost=t.fee+(kind==='LOAN_BUY'?0:t.bonus);if(a.money(c.budget)<cost)return fail('The club cannot fund this agreement.');
      ...
      return {ok:true,p,owner,terms:t,kind,swap,cost};
    }
    function proposeDeal(playerId,kind,input){
      ...
      if(owner&&['TRANSFER','SWAP'].includes(kind)){
        const credit=(swap?a.value(swap)*.8:0)+a.value(p)*t.sellOn/100*.45;
        result=a.evaluateClub({...p,club:owner},t.fee+credit);
        if(result.outcome!=='accepted')return {ok:false,message:result.title+' '+(result.counter?'Cash counter: '+a.format(Math.max(0,Math.ceil(result.counter-credit)))+'. ':'')+(result.copy||'')};
      }
    ...
    function confirmDeal(offerId){
      ...
      const q=proposal(offer.playerId,offer.kind,offer.terms);if(!q.ok)return q;const {p,owner,terms:t,swap}=q,c=a.club();
      ...
      transact(c,-q.cost,'Transfer package: '+p.name);if(owner)transact(owner,t.fee,'Transfer received: '+p.name);
      ...
      move(p,owner,c,t.fee,t);offer.status='COMPLETED';a.finishRecruitment(p.id);notice(c,'Transfer completed: '+p.name,'It’s done — '+p.name+' is ours for '+a.format(t.fee)+(swap?' plus '+swap.name:'')+', on '+a.format(t.wage)+' a week.','TRANSFERS');return success('Transfer completed.');
    }
    ...
    function processPrecontracts(date){
      for(const agreement of state().precontracts.filter(p=>p.status==='SIGNED'&&p.joinDate<=date)){
        ...
        const fee=deferred?agreement.terms.fee:0;
        if(deferred&&seller)transact(seller,fee,'Future transfer received: '+p.name);
    ...
    function crestData(input,abbr){
      const shape=input.badge==='round'?'<circle cx="50" cy="50" r="46"/>':input.badge==='diamond'?'<path d="M50 3 97 50 50 97 3 50Z"/>':'<path d="M8 6H92V58Q88 84 50 97Q12 84 8 58Z"/>';
      const svg='<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 100 100"><g fill="'+input.primary+'" stroke="'+input.secondary+'" stroke-width="6">'+shape+'</g><text x="50" y="59" text-anchor="middle" fill="'+input.secondary+'" font-family="Arial,sans-serif" font-size="24" font-weight="bold">'+abbr+'</text></svg>';
    ...
    function render(){if(!panel)return;
      ...
      let confirm='';if(confirmation){let text='';if(confirmation.action==='fire'){const p=department().staff.find(p=>p.key===confirmation.key);text=`Release ${esc(p?.name)} for ${cash((p?.wage||0)*4)} severance?`;}if(confirmation.action==='confirm-deal'){const o=(state().offers||[]).find(o=>o.id===confirmation.key);text=`Commit this ${esc(o?.kind.toLowerCase().replace('_',' '))} for ${cash((o?.terms.fee||0)+(o?.kind==='LOAN_BUY'?0:o?.terms.bonus||0))} now?`;}if(confirmation.action==='buy-option')text='Exercise this permanent purchase at the agreed fee and signing bonus?';if(confirmation.action==='recover')text='Replace this slot with its previous successful save?';confirm=`<div class="v34-confirm" role="alert"><strong>${text}</strong>${button('cancel-confirm','CANCEL')}${button('confirm-action','CONFIRM')}</div>`;}
    ...
    function staffCard(p,available=false){
      ...
      return `<article class="v36-staff-dossier" aria-label="${esc(p.name)} profile"><header class="v36-person-heading"><span class="v36-monogram" aria-hidden="true">${staffInitials(p)}</span><div><small>${esc(roles[role])} · ${available?'Candidate':'Your staff'}</small><h3>${esc(p.name)}</h3><span class="v36-person-status">${available?'Available this month':esc(staffStatus(p))}</span></div><div class="v36-quality"><strong>${p.quality}</strong><span>QUALITY / 95</span></div></header>
        <div class="v36-person-body"><section class="v36-impact"><small>${available?'WHAT THEY BRING':'STAFF CONTRIBUTION'}</small><h4>${staffImpact(role,p.quality)}</h4><p>${role==='scout'?esc((typeof p.specialism==='string'&&p.specialism.trim()?p.specialism:'GENERAL').toLowerCase())+' specialist. Assign senior reports in Transfers or youth trips in the Academy.':staffDepartments[role].description}</p>${p.course?'<p class="v36-warning">Away on a course: this staff member does not contribute until they return.</p>':''}${available?`<p class="v36-comparison">${esc(compare)}${role!=='scout'?'. Department bonuses use the strongest available staff member; they do not stack.':''}</p>`:''}</section>
    ...
    function dealsHTML(){
      ...
      <button>PROPOSE TERMS</button></form><p class="v35-term-note">${loan?'The loan fee is paid now; the buy option is voluntary. The permanent fee and bonus are only due when exercised.':'A new contract has no release clause unless you choose to negotiate one.'}</p>${agreementsHTML('active')}`;
    }
    ...
    return {state,department,market,hire,fire,trainStaff,scouts,freeScouts,sendScout,cancelMission,signProspect,declineProspect,academyCap,youthCompetition,youthTable,setYouthTeam,quoteUpgrade,upgrade,sponsorOffers,signSponsor,developmentFactor,recoveryBonus,daily,staffQuality,
      reservedPlaces,reservedWages,scheduleTransfer,proposeDeal,confirmDeal,exerciseOption,processPrecontracts,negotiateClause,onTransfer,restoreCustom,createClub,quoteSale,confirmSale,
