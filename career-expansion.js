(function(root){
  ...
  function create(a){
    ...
    function proposal(playerId,kind,input){
      ...
      if(owner&&a.humanControlledClub?.(owner))return fail('This club is controlled by another online manager. Use the standard transfer offer so they can accept or reject it themselves.');
    ...
    function bindHost(host,tab){
      ...
      host.addEventListener('click',async event=>{const tab=event.target.closest('[data-v34-tab]');if(tab){view=tab.dataset.v34Tab;feedback='';confirmation=null;render();return;}const b=event.target.closest('[data-v34-action]');if(!b)return;const action=b.dataset.v34Action,key=b.dataset.key;b.disabled=true;
        try{
          ...
          if(result){if(result.ok){if(completedAction==='hire'){staffMode='team';selectedStaffKey=key;}if(!['recover','cloud-recover'].includes(completedAction))a.save();a.refresh?.(view,completedAction);if(['confirm-deal','confirm-sale','buy-option'].includes(completedAction))a.toast(result.message);}feedback=result.message;render();}
    ...
    function savesHTML(){const store=a.store(),cloud=root.VelmoraCloudSaveManager,cloudState=cloud?.status?.(),cloudCopy=cloudState?.connected?(cloudState.setupRequired?'Cloud storage needs activating. Local autosave remains protected.':`Signed in as ${esc(cloudState.accountName||'Repo Company player')} · ${esc(cloudState.label||'cloud connected')}`):'Guest careers save to this browser. Sign in at Repo Company before launching the game to add cross-device cloud saves.';return `<section class="v34-card"><small>SAVE PROTECTION</small><h3>${cloudState?.connected?'LOCAL + PRIVATE CLOUD':'LOCAL AUTOSAVE'}</h3><p>${cloudCopy}</p></section><details class="v35-import"><summary>IMPORT A CAREER BACKUP</summary><p>Choose a slot to replace. Your other careers stay separate.</p><form data-v34-form="import" class="v34-form">${select('slot','Destination slot',[1,2,3,4,5].map(n=>option(n,'Slot '+n)).join(''))}<label>Backup file<input type="file" name="save" accept=".velmora,.json,.txt" required></label><label class="v34-check"><input type="checkbox" name="confirm" value="yes" required>Replace this slot with the uploaded backup.</label><button>IMPORT BACKUP</button></form></details>${store?.status().error?`<p class="v34-feedback">${esc(store.status().error)}. Export your active slot to keep the latest unsaved progress.</p>`:''}`;}
    function slotActions(slot,hasSave){const cloud=root.VelmoraCloudSaveManager;return `${hasSave?button('export','EXPORT BACKUP',slot):''}${a.store()?.hasBackup(slot)?button('recover','RESTORE LOCAL SAVE',slot,true):''}${cloud?.hasCloudBackup?.(slot)?button('cloud-recover','RESTORE CLOUD COPY',slot,true):''}`;}
    ...
    async function recoverCloud(slot){const cloud=root.VelmoraCloudSaveManager;if(!cloud)return fail('Cloud recovery is not available.');await cloud.restorePrevious(slot);a.reloadSlot?.(slot);return success('Previous cloud copy restored. Use Continue Career to load it.');}
