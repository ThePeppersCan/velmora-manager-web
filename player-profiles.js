...
(function(root){
  ...
  // The club a player belongs to is a link to that club, wherever it appears
  // in this overlay. Inline only, so nothing in the layout shifts.
  function clubLink(d,text){
    const label=esc(text??d.club);
    if(!d.clubId)return label;
    return `<span class="vm-club-link" data-open-club="${esc(d.clubId)}" role="link" tabindex="0" title="Open the ${esc(d.club)} club profile">${label}</span>`;
  }
  ...
  function rowHTML(label,html){return `<div class="v48-fact"><span>${esc(label)}</span><strong>${html}</strong></div>`;}
  ...
  function detailHTML(d){
    ...
    if(tab==='contract')return d.details.contract||card('Contract & recruitment',rowHTML('Club',clubLink(d))+row('Value',d.value)+row('Weekly wage',d.wage)+row('Contract',d.contract)+actionsHTML(d)+(d.canRecruit?button('Open transfer talks','data-action="recruit"',true):''));
  ...
  function render(){
    ...
    dialog.innerHTML=`<header class="v48-topbar"><div class="v48-wordmark">VELMORA<span>MANAGER</span></div><nav aria-label="Career navigation">${['Central','Squad','Transfers','Matchday','Season','Office'].map(n=>`<button type="button" data-nav="${n.toLowerCase()}" ${d.liveMatch?'disabled':''}>${n}</button>`).join('')}</nav></header>
      ...
      <main class="v48-profile-main" tabindex="-1" aria-label="${esc(d.name)} · ${tab}">${tab==='overview'?overviewHTML(d):`<div class="v48-detail-head"><img src="${esc(d.sprite)}" alt=""/><div><span>${clubLink(d)} · ${esc(d.role)}</span><h1>${esc(d.name)}</h1><p>${esc(d.stage)}</p></div><b>${esc(d.ovr)}<small>OVR</small></b></div><div class="v48-detail-body">${detailHTML(d)}</div>`}</main></div></div>`;
