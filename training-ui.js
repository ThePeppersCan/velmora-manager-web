...
(() => {
  ...
  function render(root,state){
    ...
    const toolbar=root.querySelector('.tr-toolbar');
    toolbar?.querySelector('.tr-toolbar-note')?.remove();
    toolbar?.insertAdjacentHTML('beforeend',`<label><span>Club focus</span><select data-tr-club-focus data-tr-focus="clubFocus" aria-label="Weekly club training focus">${options((data.trainingFocuses||['Balanced','Attack','Defence','Fitness','Technical','Youth','Rest']).map(value=>[value,value]),data.trainingFocus)}</select></label>`);
  ...
  function mount(root,api){
    ...
    if(!state){
      ...
      root.addEventListener('change',event=>{
        ...
        else if(Object.prototype.hasOwnProperty.call(input.dataset,'trClubFocus')){state.api.setClubFocus(input.value);state.message='Weekly club focus updated.';}
