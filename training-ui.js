...
(() => {
  ...
  function detail(row,data){
    ...
    return `<section class="tr-detail" id="trPlayerDetail" aria-label="Selected player training plan"><header><span class="tr-eyebrow">ASSISTANT COACH</span><h3>${escape(row.advice.title)}</h3><p>${escape(row.advice.copy)}</p><button type="button" data-tr-profile="${escape(row.player.id)}" data-tr-focus="profile">Open ${escape(row.player.name)}’s profile ↗</button></header><div class="tr-detail-body"><div class="tr-section-title"><h4>Preparation schedule</h4><span>${days===0?'Match today':days===null?'Next seven days':`${days} day${days===1?'':'s'} to prepare`}</span></div><div class="tr-schedule">${schedule}</div><div class="tr-note"><strong>${row.player.name?escape(row.player.name)+' · ':''}${escape(planName(row.plan))}</strong><p>${T.plans[row.plan]?.copy||'Medical and matchday preparation take priority over the selected schedule.'} Hard sessions switch to recovery below 60% fitness.</p></div></div></section>`;
  }
