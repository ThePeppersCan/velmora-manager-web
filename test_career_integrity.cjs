...
const key='velmora-manager-career-v3-2',codec=r.context.VelmoraSaveCodec;
...
check('loan return restores unique ownership and salary commitments',()=>{
 const parentSquad=q.getSquad(c),template=parentSquad.find(p=>!p.captain)||parentSquad[0];let fillerIndex=0;
 while(parentSquad.length<13){const filler=JSON.parse(JSON.stringify(template));filler.id=`QA-RETURN-FILLER-${++fillerIndex}`;filler.name=`Return Filler ${fillerIndex}`;filler.captain=false;filler.clubId=c.id;filler.ownerClubId=c.id;filler.parentClubId=null;filler.onLoan=false;parentSquad.push(filler);q.addPlayerToLineup(c,filler.id,'reserve');}
 assert(q.returnLivingLoan(loan,'2027-06-30'));assert.equal(loan.status,'RETURNED');assert.equal(loanPlayer.onLoan,false);assert.equal(loanPlayer.seniorRegistrationStatus,'AWAITING_REGISTRATION');assert(q.getSquad(c).some(p=>p.id===loanPlayer.id));assert(!q.getSquad(borrower).some(p=>p.id===loanPlayer.id));const all=q.state().clubs.flatMap(c=>q.getSquad(c));assert.equal(all.filter(p=>p.id===loanPlayer.id).length,1);
 q.renderSquad();assert(String(r.nodes.get('reservesGrid')?.innerHTML||'').includes(loanPlayer.name));assert(String(r.nodes.get('reservesGrid')?.innerHTML||'').includes('AWAITING REGISTRATION'));
 const registeredTarget=parentSquad.find(p=>p.id!==loanPlayer.id&&q.playerLineupPosition(c,p.id));assert(registeredTarget);assert(q.swapLineupPlayers(c,loanPlayer.id,registeredTarget.id));assert(q.playerLineupPosition(c,loanPlayer.id));assert.equal(q.playerLineupPosition(c,registeredTarget.id),null);
});
check('quick sim rejects injured starters without committing a fixture',()=>{
 const p=q.activeStarters(c)[0],other=q.state().clubs.find(x=>x.id!==c.id);p.injured=true;p.injuryDaysRemaining=10;const f={fixtureId:'INJURED-QA',homeClubId:c.id,awayClubId:other.id,date:'2026-08-10',type:'LEAGUE'};assert.equal(q.simulateUserFixture(f,'QUICK SIM'),null);assert(!f.played);p.injured=false;p.injuryDaysRemaining=0;
});
